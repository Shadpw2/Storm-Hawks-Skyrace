...
COLOURS = dict(((x, getattr(CharCreationColours, x)) for x in ('eyeColour', 'skinColour',
                                                               'hairColour1', 'hairColour2',
                                                               'clothesColour1',
                                                               'clothesColour2')))
...
class Skyride(Vehicle):
    ...
    def __init__(self):
        ...
        self._jets = [ (('_').join((name, side)), BigWorld.Model('vehicles/vfx/vfx-skim-%s001.model' % name), action) for (name, side, action) in (
         ('pipejet', 'L', 'mediumspeed'),
...
class CharacterModel(object):
    ...
    def _fixModel(self):
        if self.emoting():
            ...
            del self._fixface
        m = BigWorld.Model(path(self._species, ('female', 'male')[self._gender], 'body', 'body'), path(self._species, ('female',
                                                                                                                       'male')[self._gender], 'body', 'head'))
