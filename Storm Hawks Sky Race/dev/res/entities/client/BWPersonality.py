...
def save_character_data(player_name, updates):
    ...
    NOTE: 'gold' and 'inventory' are special-cased to write into the
    character's 'offline' block (char['offline']['gold'], etc.) rather
    than the top level of the record. Every other system that reads
    gold/inventory -- EnterOfflineWorld, ShopMode, Inventory.py's
    Bitcasters.OfflineSave -- reads it from there, nested under
    'offline'. Writing it at the top level (what this used to do)
    created a field nothing else ever looks at, which is why gold
    updates via this function/add_gold() never actually stuck: the
    live in-memory value changed, but the saved value everyone else
    reads back out stayed exactly where it was. Any other key in
    `updates` still updates the top level as before.
    """
    try:
        file_chars = []
            ...
            if char.get('name') == player_name:
                ...
                for (key, value) in updates.items():
                    if key == 'gold':
                        ...
                        # `value` is expected to be the live
                        # {slotID: InventoryItem} shape -- split it
                        # into equipped (slot 0) vs bag (slot >=
                        # WEAR_SLOT_COUNT) and serialize both, the same
                        # way Bitcasters.OfflineSave.save_offline_state()
                        # already does, so this stays consistent with
                        # every other save path.
                        try:
                            from InventoryItem import serialize, WEAR_SLOT_COUNT
                            equipped = dict((k, v) for (k, v) in value.items() if k < WEAR_SLOT_COUNT)
                            bag = dict((k, v) for (k, v) in value.items() if k >= WEAR_SLOT_COUNT)
                            offline['inventory_equipped'] = serialize(equipped)
                            offline['inventory_bag'] = serialize(bag)
                        except Exception, e:
                            print "[save_character_data] failed to serialize inventory update:", e
                    else:
                        remaining[key] = value

                char['offline'] = offline
                char.update(remaining)
                found = True
