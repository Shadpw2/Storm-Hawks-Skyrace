...
# Embedded file name: /entities/common/Lib/zipfile.py
# Compiled at: 2008-12-16 09:37:55
"""
Read and write ZIP files.
"""
import struct, os, time, sys, binascii, cStringIO
try:
    import zlib
except ImportError:
    zlib = None
...
def _EndRecData(fpin):
    ...
    if data[0:4] == stringEndArchive and data[-2:] == '\x00\x00':
        ...
        if endrec[-4] == -1 or endrec[-4] == 4294967295L:
            return _EndRecData64(fpin, -22, endrec)
        return endrec
    END_BLOCK = min(filesize, 4096)
...
class ZipInfo(object):
    ...
    def FileHeader(self):
        ...
        header = struct.pack(structFileHeader, stringFileHeader, self.extract_version, self.reserved, self.flag_bits, self.compress_type, dostime, dosdate, CRC, compress_size, file_size, len(self.filename), len(extra))
