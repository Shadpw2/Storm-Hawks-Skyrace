...
# Embedded file name: /entities/common/Lib/lib-tk/FileDialog.py
    ...
    def ok_command(self):
        ...
        if os.path.exists(file):
            if os.path.isdir(file):
                self.master.bell()
                return
            d = Dialog(self.top, title='Overwrite Existing File Question', text='Overwrite existing file %r?' % (file,), bitmap='questhead', default=1, strings=('Yes',
                                                                                                                                                                 'Cancel'))
            if d.num != 0:
                return
        else:
