...
# Embedded file name: /entities/common/Lib/lib-tk/Canvas.py
...
class CanvasItem:
    ...
    def keys(self):
        if not hasattr(self, '_keys'):
            self._keys = map((lambda x, tk=self.canvas.tk: tk.splitlist(x)[0][1:]), self.canvas.tk.splitlist(self.canvas._do('itemconfigure', (
             self.id,))))
        return self._keys
