...
# Embedded file name: /entities/common/Lib/lib-tk/turtle.py
...
class RawPen():
    ...
    def write(self, text, move=False):
        ...
        (x, y) = self._position
        x = x - 1
        item = self._canvas.create_text(x, y, text=str(text), anchor='sw', fill=self._color)
    ...
    def fill(self, flag):
        ...
        if self._filling:
            ...
            if len(path) > 2:
                item = self._canvas._create('polygon', path, {'fill': self._color, 'smooth': smooth})
                self._items.append(item)
        self._path = []
        self._filling = flag
        if flag:
    ...
    def _goto(self, x1, y1):
        ...
        if self._drawing:
            if self._tracing:
                ...
                nhops = int(distance)
                item = self._canvas.create_line(x0, y0, x0, y0, width=self._width, capstyle='round', fill=self._color)
                try:
                ...
                item = self._canvas.create_line(x0, y0, x1, y1, width=self._width, capstyle='round', fill=self._color)
            self._items.append(item)
        self._draw_turtle()
    ...
    def speed(self, speed):
        ...
        try:
            ...
            raise ValueError('%r is not a valid speed. speed must be one of %s' % (
             speed, speeds))
    ...
    def _draw_turtle(self, position=[]):
        ...
        self._delete_turtle()
        self._arrow = self._canvas.create_line(x - dx, y + dy, x, y, width=self._width, arrow='last', capstyle='round', fill=self._color)
        self._canvas.update()
...
def write(arg, move=0):
    _getpen().write(arg, move)
...
def delay(delay):
    return _getpen().delay(delay)


def speed(speed):
    return _getpen().speed(speed)
...
def demo2():
    ...
    while abs(position()[0] - turtle.position()[0]) > 4 or abs(position()[1] - turtle.position()[1]) > 4:
        turtle.forward(3.5)
        turtle.left(0.6)
        setheading(towards(turtle))
        forward(4)
