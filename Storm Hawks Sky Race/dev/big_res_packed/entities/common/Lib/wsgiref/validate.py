...
# Embedded file name: /entities/common/Lib/wsgiref/validate.py
...
def check_environ(environ):
    assert_(type(environ) is DictType, 'Environment is not of the right type: %r (environment: %r)' % (
    ...
    for key in ['HTTP_CONTENT_TYPE', 'HTTP_CONTENT_LENGTH']:
        assert_(key not in environ, 'Environment should not have the key: %s (use %s instead)' % (
         key, key[5:]))
    ...
    for key in environ.keys():
        if '.' in key:
            continue
        assert_(type(environ[key]) is StringType, 'Environmental variable %s is not a string: %r (value: %r)' % (
...
def check_input(wsgi_input):
    for attr in ['read', 'readline', 'readlines', '__iter__']:
        assert_(hasattr(wsgi_input, attr), "wsgi.input (%r) doesn't have the attribute %s" % (
         wsgi_input, attr))


def check_errors(wsgi_errors):
    for attr in ['flush', 'write', 'writelines']:
        assert_(hasattr(wsgi_errors, attr), "wsgi.errors (%r) doesn't have the attribute %s" % (
         wsgi_errors, attr))
...
def check_headers(headers):
    assert_(type(headers) is ListType, 'Headers (%r) must be of type list: %r' % (
     headers, type(headers)))
    header_names = {}
    for item in headers:
        assert_(type(item) is TupleType, 'Individual headers (%r) must be of type tuple: %r' % (
         item, type(item)))
        assert_(len(item) == 2)
        (name, value) = item
