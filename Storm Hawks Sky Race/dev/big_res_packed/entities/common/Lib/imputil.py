...
# Embedded file name: /entities/common/Lib/imputil.py
...
"""
import imp, sys, __builtin__, struct, marshal
__all__ = [
...
class Importer:
    ...
    def _import_fromlist(self, package, fromlist):
        ...
        for sub in fromlist:
            if sub != '*':
                if not hasattr(package, sub):
                    subname = '%s.%s' % (package.__name__, sub)
                    submod = self._import_one(package, sub, subname)
                    raise submod or ImportError, 'cannot import name ' + subname
