...
# Embedded file name: /entities/common/Lib/idlelib/AutoCompleteWindow.py
...
KEYPRESS_SEQUENCES = ('<Key>', '<Key-BackSpace>', '<Key-Return>', '<Key-Tab>', '<Key-Up>',
                      '<Key-Down>', '<Key-Home>', '<Key-End>', '<Key-Prior>', '<Key-Next>')
...
class AutoCompleteWindow:
    ...
    def _change_start(self, newstart):
        i = 0
        while i < len(self.start) and i < len(newstart) and self.start[i] == newstart[i]:
            i += 1
    ...
    def _complete_string(self, s):
        ...
        last = i - 1
        i = len(s)
        while len(self.completions[first]) > i and len(self.completions[last]) > i and self.completions[first][i] == self.completions[last][i]:
            i += 1
    ...
    def winconfig_event(self, event):
        ...
        acw.wm_geometry('+%d+%d' % (x + self.widget.winfo_rootx(),
         y + self.widget.winfo_rooty() - acw.winfo_height()))
    ...
    def keypress_event(self, event):
        ...
        if keysym != 'Tab':
            self.lastkey_was_tab = False
        if (len(keysym) == 1 or keysym in ('underscore', 'BackSpace') or self.mode == AutoComplete.COMPLETE_FILES and keysym in ('period',
                                                                                                                                 'minus')) and not state & ~MC_SHIFT:
            if len(keysym) == 1:
            ...
            self.hide_window()
            return
        elif self.mode == AutoComplete.COMPLETE_ATTRIBUTES and keysym in ('period',
                                                                          'space',
                                                                          'parenleft',
                                                                          'parenright',
                                                                          'bracketleft',
                                                                          'bracketright') or self.mode == AutoComplete.COMPLETE_FILES and keysym in ('slash',
                                                                                                                                                     'backslash',
                                                                                                                                                     'quotedbl',
                                                                                                                                                     'apostrophe') and not state & ~MC_SHIFT:
            cursel = int(self.listbox.curselection()[0])
            if self.completions[cursel][:len(self.start)] == self.start and (self.mode == AutoComplete.COMPLETE_ATTRIBUTES or self.start):
                self._change_start(self.completions[cursel])
            self.hide_window()
            return
            ...
            if self.lastkey_was_tab:
                ...
                return
        elif reduce((lambda x, y: x or y), [ keysym.find(s) != -1 for s in ('Shift',
                                                                            'Control',
                                                                            'Alt',
                                                                            'Meta',
                                                                            'Command',
