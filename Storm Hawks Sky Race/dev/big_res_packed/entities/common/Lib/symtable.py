...
# Embedded file name: /entities/common/Lib/symtable.py
# Compiled at: 2008-12-16 09:37:52
"""Interface to the compiler's internal symbol tables"""
import _symtable
from _symtable import USE, DEF_GLOBAL, DEF_LOCAL, DEF_PARAM, DEF_STAR, DEF_DOUBLESTAR, DEF_INTUPLE, DEF_FREE, DEF_FREE_GLOBAL, DEF_FREE_CLASS, DEF_IMPORT, DEF_BOUND, OPT_IMPORT_STAR, OPT_EXEC, OPT_BARE_EXEC
import weakref
__all__ = [
...
def is_free(flags):
    if flags & (USE | DEF_FREE) and flags & (DEF_LOCAL | DEF_PARAM | DEF_GLOBAL):
        return True
...
class SymbolTable:
    ...
    def get_type(self):
        ...
        if self._table.type == _symtable.TYPE_CLASS:
            return 'class'
        assert self._table.type in (1, 2, 3), 'unexpected type: %s' % self._table.type
    ...
    def __check_children(self, name):
        return [ newSymbolTable(st, self._filename) for st in self._table.children if st.name == name
               ]

    def get_children(self):
        return [ newSymbolTable(st, self._filename) for st in self._table.children
               ]
...
class Symbol:
    ...
    def is_global(self):
        return bool(self.__flags & DEF_GLOBAL or self.__flags & DEF_FREE_GLOBAL)
    ...
    def is_free(self):
        if self.__flags & (USE | DEF_FREE) and self.__flags & (DEF_LOCAL | DEF_PARAM | DEF_GLOBAL):
            return True
