...
# Embedded file name: /entities/common/Lib/distutils/ccompiler.py
...
class CCompiler():
    ...
    def _check_macro_definitions(self, definitions):
        ...
        for defn in definitions:
            if not (type(defn) is TupleType and (len(defn) == 1 or len(defn) == 2 and (type(defn[1]) is StringType or defn[1] is None)) and type(defn[0]) is StringType):
...
compiler_class = {'unix': ('unixccompiler', 'UnixCCompiler',
   ...
   'bcpp': (
          ...
          'Borland C++ Compiler'), 
   'mwerks': (
            'mwerkscompiler', 'MWerksCompiler',
            'MetroWerks CodeWarrior'), 
   'emx': (
         'emxccompiler', 'EMXCCompiler',
