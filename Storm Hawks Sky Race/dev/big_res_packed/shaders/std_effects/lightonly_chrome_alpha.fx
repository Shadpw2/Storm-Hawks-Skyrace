	...
	uniform int nSpots )
{
	...
	diffuse.a = fresnel( -normalize(eye), normalize(input.normal), fresnelExp, fresnelConstant ) * 2.0 * reflectionAmount;
...
OutputDiffuseLighting3 vs_mainStaticLighting( VertexXYZNUV_D input, 
	uniform int nDirectionals, 
	uniform int nPoints, 
	uniform int nSpots )
{
