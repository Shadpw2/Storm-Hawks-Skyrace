...
<
	...
	string UIDesc = "The scale of the bumpiness";
	float UIMin = 0;
	float UIMax = 1;
	int UIDigits = 1;
> = 0.2;
...
OutputCharacterLighting vs_main( VertexXYZNUVIIIWWTB input,
...
OutputCharacterLightingSpot vs_mainSpot( VertexXYZNUVIIIWWTB input,
	uniform int nDirectionals, 
	uniform int nPoints, 
	uniform int nSpecularDirectionals, 
	uniform int nSpecularPoints )
	...
	uniform int nSpecularPoints ) : COLOR0
{
	...
	normal = normalize( float3(1,bumpScale,1) * normal );
...
OutputCharacterLighting characterLighting_maxpreview_vs2( VertexXYZNUVTB input)
{
	OutputCharacterLighting output = (OutputCharacterLighting)0;
	DirectionalLight dl;
	...
	output.attenuation.x = directionalBumpLight( tsMatrix, dl, output.dLight1.xyz );
	output.attenuation.y = 0.f;

	float3 camp = mul( viewInverse[3].xyz, worldInverse );
	float3 eye = normalize(camp - input.pos);
	output.attenuation.z = directionalSpecBumpLight( eye, tsMatrix, dl, output.sLight1.xyz );
	output.attenuation.w = 0;
...
float4 characterLighting_maxpreview_ps2( OutputCharacterLighting input ) : COLOR0
{
	//  Output constant color:
	float4 diffuseMap = tex2D( diffuseSampler, input.tc );
	float3 specularMap = tex2D( specularSampler, input.tc );
	...
	float4 colour;
	colour.xyz = diffuseColour + specularColour;
