...
#ifdef MOD2X
	colour.xyz = (skyMap * diffuseColour * (1 + diffuseLightExtraModulation) ) + specularColour;
#else
	colour.xyz = skyMap * diffuseColour + specularColour;
#endif
	...
	uniform int nSpecularPoints ) : COLOR0
{
	...
	specular1Colour = spotLights[0].colour.xyz * materialSpecular;

	if (nSpecularPoints > 0)
		specular2Colour = specularPointLights[0].colour.xyz * materialSpecular;
	else if (nSpecularDirectionals > 0)
		specular2Colour = specularDirectionalLights[0].colour.xyz * materialSpecular;
	else
		s2 = false;
...
OutputDiffuseCharacterLighting diffuseCharacterLighting_vs1( VertexXYZNUVITB input,
	uniform int nDirectionals, 
	uniform int nPoints )
{
	OutputDiffuseCharacterLighting output = (OutputDiffuseCharacterLighting)0;
	...
	if (nDirBump > 1)
		...
		output.attenuation.xyz = pointBumpLight( worldPos, tsMatrix, pointLights[0], output.dLight1.xyz );
	else
		output.attenuation.xyz = 0;

	if (nDirBump > 0)
		output.attenuation.w = directionalBumpLight( tsMatrix, directionalLights[0], output.dLight2.xyz );
	else if (nPointBump > 1)
		output.attenuation.w = pointBumpLight( worldPos, tsMatrix, pointLights[1], output.dLight2.xyz );
	else
		output.attenuation.w = 0;
	return output;
}
...
OutputSpecularCharacterLightingAlpha specularCharacterLighting_vs1( VertexXYZNUVITB input,
	uniform int nSpecularDirectionals, 
	uniform int nSpecularPoints )
{
	OutputSpecularCharacterLightingAlpha output = (OutputSpecularCharacterLightingAlpha)0;
	...
	if (nPointSpecBump > 0)
		output.attenuation.xyz = pointSpecBumpLight( worldPos, eye, tsMatrix, specularPointLights[0], output.sLight.xyz );
	else if (nDirSpecBump > 0)
		output.attenuation.xyz = directionalSpecBumpLight( eye, tsMatrix, specularDirectionalLights[0], output.sLight.xyz );
	else 
		output.attenuation.xyz = 0;
	return output;
}
...
float4 diffuseCharacterLighting_ps1( OutputDiffuseCharacterLighting input,
							uniform int nBumpPointLights) : COLOR0
{
	float4 diffuseMap = tex2D( diffuseSampler, input.tc );
	float3 normal = (tex2D( normalSampler, input.ntc ) * 2 ) - 1;
	float3 diffuseDir1 = (texCUBE( normalisationSampler, input.dLight1 ) * 2) - 1;
	float3 diffuseDir2 = (texCUBE( normalisationSampler, input.dLight2 ) * 2) - 1;
	...
	diffuse.xyz += saturate(dot(normal, diffuseDir1)) * input.attenuation.xyz * d1Colour;
	diffuse.xyz += saturate(dot(normal, diffuseDir2)) * input.attenuation.w * d2Colour;
...
float4 specularCharacterLighting_ps1( OutputSpecularCharacterLightingAlpha input, 
							uniform bool point ) : COLOR0
{
	float3 normal = (tex2D( normalSampler, input.ntc ) * 2) - 1;
	float3 specularDir = (texCUBE( normalisationSampler, input.sLight ) * 2) - 1;
	float3 glow = (tex2D( glowSampler, input.tc )) * glowFactor;
	float4 specular;
	...
	float3 colour = specularColour * input.attenuation.xyz;
		
	float alpha = tex2D(diffuseSampler, input.atc).a;
