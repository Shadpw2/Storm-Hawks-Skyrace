...
OutputCharacterLightingEnv vs_main( VertexXYZNUVITB input,
...
OutputSpecularCharacterLighting specularCharacterLighting_vs1( VertexXYZNUVITB input,
	uniform int nSpecularDirectionals, 
	uniform int nSpecularPoints )
{
	OutputSpecularCharacterLighting output = (OutputSpecularCharacterLighting)0;
...
float4 specularCharacterLighting_ps1( OutputSpecularCharacterLighting input, 
...
OutputReflection reflection_vs1( VertexXYZNUVITB input )
{
	...
	float4 ut = float4( transformPos( float4(1,0,0,1), input.index ), 1) * 0.5;
	float4 vt = float4( transformPos( float4(0,-1,0,1), input.index ), 1) * 0.5;
